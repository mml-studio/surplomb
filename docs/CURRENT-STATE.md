...
>   — Véhicules partagés (FR) occupies its slot with TWO collections (every dot,
>   stations first and vehicles after them, below; the few vehicle pins above),
>   registered in that order, so a pin paints over the dots it stands among
>   without either escaping the layer's slot.
