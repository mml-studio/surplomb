...
| Événements routiers 🇫🇷 ⚠ | Bison Futé / Tipi *Événementiel-DIR* (DATEX II v2 `SituationPublication`) — every accident, rockfall, closure, roadworks order and diversion the **Directions interdépartementales des routes** declare on the non-conceded national network. **One marker per *situation*, not per record**: an accident and the two lanes it blocked are one incident, with its consequences counted on the card. Planned is not happening — future orders are held apart from what is live now. **Hosted build: the conceded motorways too** — with `BISON_FUTE_RESTRICTED_USER`/`_PASSWORD` set, the proxy starts a background poller on the licensed *Action b* stream (index every 5 min, files one at a time at ≤ 2/s, full ~24 h window replayed on a cold start), keeps only live situations in `.gev-cache/bison-fute/action-b.json`, and merges them on situation id with the open feed (later version wins; nothing served until the first replay has read the whole window). Those cards read « Information fournie par <producer> · mise à jour <time> » and the row says *RRN, autoroutes concédées comprises*; `/api/bison-fute/status` reports `actionB` | `src/data/roadEventsFrance.js`, `src/data/bisonFuteFeed.js`, `src/data/bisonFuteActionB.js` | `/api/bison-fute/events` (keyless; Action b where the login is set) | 5 min |
