...
| Bikeshare 🚲 | GBFS (Lyft + BCycle, plus Vélib', Vélo'v, vélÔToulouse, Le Vélo TBM). A dock is a dot ringed in its network's hue and filled with that same hue as far as it is full (`mobilityDockFill`: solid, tint, empty ring); while the shared fleets draw their city-wide groups, its available bikes are counted into them and the dot is not drawn (`mobilityDockBridge.js`). Its block in the « Mobilités partagées » key names each network in view in its ring colour and explains the dock fill; it takes the row's fanned-out `operator` and `kinds` filters and hides docks by `point.show`, no rebuild | `src/data/bikeshare.js` | `/api/gbfs` | 60s |
