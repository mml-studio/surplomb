...
/** Most env vars accepted in one save. The registry defines thirteen. */
...
export const KEY_SETUP_KEYS = Object.freeze([
  ...
  // Ahead of the metered voice keys on this fork, and ahead of where upstream
  // ranks it: an EEA-billed Google project is refused Photorealistic 3D Tiles
  // and satellite imagery outright, and a free ion token is the ONLY route to
  // Google's 3D globe from France. It also carries Bing imagery and world
  ...
  // This fork's second voice route: browser speech in and out with one text
  // model between them. An alternative to the OpenAI key, never an addition —
  ...
  // France-only, and free: no card, no quota worth the name. Without it the
  // layer still draws the whole 171-unit fleet from the register it ships; the
  ...
  // The vigilance layer WORKS without this: Météo-France mirrors the identical
  // product on data.gouv.fr, ~20 s behind. The key buys the contracted API
