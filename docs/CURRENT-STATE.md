...
- **adsb.lol pacing (`src/adsbLolFeed.js`).** Every api.adsb.lol request — French circles, regional circles, `/v2/mil` — leaves through one queue, ≥ 20 s apart: waiters first, then never-fetched jobs, then the oldest answer; a job not wanted for 75 s stops refreshing; a 429 or 420 pauses the queue 60 s; answers older than 10 min are not served. A regional circle is shared by any anchor within 100 NM of its centre. `/api/adsblol/mil` serves the queue's newest copy (`X-ADS-B-Cache`, `X-ADS-B-Age-Seconds`) and waits at most 25 s when it has none.
