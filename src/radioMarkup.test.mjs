...
test('no unchanged Realtime tool definition drifts silently', () => {
  ...
  const TOUCHED = new Set([
    ...
    'set_map_stack',
    // Maritime layers: the NGA World Port Index pack (`local-ports`) and the
    // NOAA NDBC buoy feed (`marine-buoys`) joined the layer enums. Buoys are
    // deliberately ABSENT from get_entity_context — that layer registers no
    // entity context, so listing it would promise the model a lookup that
    // returns nothing.
...
test('Radio is nested inside Context with separate disclosure and power controls', () => {
  ...
  assert.match(html, /aria-label="Lecture radio"/);
  ...
  assert.match(html, /id="context-radio-mini"[^>]*aria-label="Commandes Radio compactes"[^>]*hidden/);
  ...
  assert.match(html, /id="context-radio-mini-close-btn"[\s\S]{0,200}?aria-label="Fermer les commandes Radio compactes"/);
  assert.match(html, /id="context-radio-mini-(?:prev|play|next)-btn"/);
  assert.match(html, /id="context-radio-mini-volume"/);
  assert.match(html, /id="cockpit-radio-panel"[^>]*aria-label="Commandes Radio compactes du cockpit"[^>]*hidden/);
  ...
  assert.match(html, /id="radio-tuner-band-label"[^>]*>BANDE DE L’ANNUAIRE/);
  assert.match(html, /id="radio-tuner-slider"[^>]*type="range"/);
  assert.match(html, /id="radio-tuner-needle"[^>]*aria-hidden="true"/);
  assert.match(html, /S’ACCROCHE AUX STATIONS DISPONIBLES/);
  assert.match(html, /class="radio-tuner-scale" aria-hidden="true"><\/div>/);
  assert.match(html, /ANNUAIRE : RADIO BROWSER/);
  assert.match(html, /Le son vient directement du diffuseur/);
