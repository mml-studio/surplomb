...
const defaultAllow = new RegExp(PROPER_NOUN_LIST.map(escapeRegExp).join('|'), 'gu');
...
function allowPattern(extra) {
  ...
  if (!pattern) {
    ...
    pattern = new RegExp(names.map(escapeRegExp).join('|'), 'gu');
...
export function findFrench(text, { allow = [], formatting = true } = {}) {
  ...
  for (const [token] of stripped.matchAll(WORD)) {
